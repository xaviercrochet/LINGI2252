...
\begin{lstlisting}
(MooseModel root allModels first) allClasses 
	select: [:each | (each METRICNAME OPERATOR THRESHOLD)
\end{lstlisting}
Avec
\begin{itemize}
\item METRICNAME - The name of the Metric
\item OPERATOR - The operator for the comparaison
\item THRESHOLD - The value of the threshold
 \end{itemize}

\subsection{Code duplication}
The metrics we have to handle are \textit{NumberOfExternalDuplications} and \textit{NumberOfInternalDuplications}.
