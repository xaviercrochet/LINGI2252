...
And compute the following graph, displaying the number of class aving their attribute less or equal than the value.\\

We see that the class having the highest cohesion value (1.47) is GLMUpdateMorphicTest and has only 96 lines of codes. Furthemore, when querying for class having tightClassCohesion less than 0.1, we have 197 classes, GLMPresentation and his whole family (GLMListingPresentation,  GLMWizard GLMBrowser, ...) Other elephant classes such as GLMUITheme, GLMWhiteSpaceTheme. \\ 

Thoses classes where also identified in the first report as elephant classes. There is work to do on this part, to increase class cohesion by refactoring them into several smaller classes.\\ 
